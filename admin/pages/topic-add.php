...
<?php
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Chủ đề</h1><div id='noti-action'></div>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                                ...
                                <div class="col-md-10 col-md-push-1">
                                    <form id="change" name="regis" method="POST" onSubmit="javascript:return validate();">
                                        <div class="form-group row">
                                            <label for="title-topic" class="col-md-3 control-label">Tên chủ đề mới</label>
                                            <div class="col-md-9">
                                                <input class="form-control" type="text" name="title-topic" id="title-topic">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-9">
                                                <select class="form-control" name="state" id="state">
                                                    <option>--Chọn trạng thái cho chủ đề-- </option>
                                                    <option value="1">Hiển thị </option>
                                                    <option value="0">Lưu trữ </option>
                                                </select>
                                            </div>
                                        </div>
                                        <button type="submit" class="btn btn-pop center-block" value="add-topic" name="btnAdd" id="btnAdd"><b>Thêm chủ đề</b></button>
                                    </form>
                                    <?php
                                        include( "control/configure.php");
                                        if (isset($_REQUEST['btnAdd'])&&isset($_REQUEST['title-topic'])&&isset($_REQUEST['state'])) {
                                            $title=$_REQUEST['title-topic'];
                                            $state=$_REQUEST['state'];
                                            $color=$_REQUEST['color'];
                                            $sql="INSERT INTO topic(nametopic,state,color) VALUES ('{$title}','{$state}','{$color}')";
                                            if(mysqli_query($con,$sql)) {
                                                ?>
                                                <script type="text/javascript">
                                                    document.getElementById("noti-action").innerHTML = "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>Thêm chủ đề mới thành công.</div>";
                                                </script>
                                                <?php
                                            }else {
                                                ?>
                                                <script type="text/javascript">
                                                    document.getElementById("noti-action").innerHTML = "<div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>Thêm chủ đề mới không thành công.</div>";
                                                </script>
                                                <?php
