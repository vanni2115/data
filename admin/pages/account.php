                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="mytable">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM user,role WHERE user.idrole=role.idrole";
                                        $result = mysqli_query($con, $sql);
                                        $num_rows = mysqli_num_rows($result);
                                        while($row=mysqli_fetch_array($result)){
                                        echo "<tr class='odd gradeX'>
                                                <td><a href='../../profile.php?iduser={$row['iduser']}'>".$row['fullname']."</a></td>
                                                <td>".$row['email']."</td>
                                                <td>".$row['namerole']."</td>
                                                <td>
                                                    <div class='pull-right action-buttons'>
                                                        <a href='edit-account.php?iduser={$row['iduser']}' name='btnChange' value='change'><span class='glyphicon glyphicon-pencil'></span></a>
                                                        <a href='delete-account.php?iduser={$row['iduser']}' name='btnDel' value='del' class='trash'><span class='glyphicon glyphicon-trash' style='color:rgb(209, 91, 71)'></span></a>";
                                                            if($row['state']==1){
                                                                echo "<a href='update-state.php?iduser={$row['iduser']}' class='flag' title='Hiển thị'><span class='glyphicon glyphicon-flag' style='color:rgb(248, 148, 6);'></span></a>";
                                                            }else{
                                                                echo "<a href='update-state.php?iduser={$row['iduser']}' class='flag' title='Đang lưu trữ'><span class='glyphicon glyphicon-flag' style='color:rgba(50,50,50,.5);'></span></a>";
                                                            }
